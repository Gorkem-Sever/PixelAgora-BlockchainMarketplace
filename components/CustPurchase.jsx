...
const CustPurchase = ({ product, timestamps }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const params = {
      ...
      amount:
        ...
        (product?.price * quantity * securityFee) / 100,
    };
    ...
    >
      ...
      {localListings > 0 ? (
        <>
          ...
          <button
            type="submit"
            className="p-2 border-none bg-orange-500 text-white w-full rounded-md focus:outline-none focus:ring-0"
          >
            Buy
          </button>
        </>
      ...
      <Link href={`/product/purchases/${product?.id}`} className="text-orange-400 dark:text-orange-400">
