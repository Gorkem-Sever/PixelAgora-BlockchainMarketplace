  ...
  return (
    <div className="w-full flex justify-between items-center my-3 bg-gray-100 dark:bg-gray-800 p-3 rounded-lg shadow-md">
      <div className="flex items-center">
        <Identicon
          string={isSeller ? purchase.customer : product.owner}
          size={30}
        ...
        <div className="flex flex-col ml-3">
          ...
          <span className="text-gray-500 dark:text-gray-400 text-sm">{truncate(isSeller ? purchase.customer : product.owner, 4, 4, 11)}</span>
        </div>
      </div>
      ...
      <div className="flex space-x-2 items-center">
        <Identicon
          string={isSeller ? product.owner : purchase.customer}
          size={30}
          className="rounded-full shadow-gray-500 shadow-sm"
        />
        <span className="text-gray-500 dark:text-gray-400 text-sm">{truncate(isSeller ? product.owner : purchase.customer, 4, 4, 11)}</span>
      </div>
      ...
      {!isSeller && purchase.checked && !purchase.cancelled && (
        <button
          className="p-2 bg-green-400 hover:bg-green-300 text-white font-medium italic rounded-full text-sm px-4"
        >
          Approved
        </button>
      )}
      ...
      {isSeller && !purchase.cancelled && (
        <button
          className="p-2 bg-orange-500 hover:bg-green-400 text-white font-medium italic rounded-full text-sm px-4"
        >
          Purchase Complete
        </button>
      )}
      ...
      {purchase.cancelled && (
        <button
          className="p-2 bg-red-400 hover:bg-red-300 text-white font-medium italic rounded-full text-sm px-4"
        >
          Cancelled
        </button>
      )}
