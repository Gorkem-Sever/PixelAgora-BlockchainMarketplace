...
const Purchases = ({ productData, purchasesData }) => {
  ...
  if (!product) {
    return <div className="text-center mt-4 text-gray-700 dark:text-gray-300">Loading...</div>
  }

  const isSeller = product?.owner === address
  const userPurchases = isSeller ? purchases : purchases.filter(purchase => purchase.customer === address)
...
export const getServerSideProps = async (context) => {
  ...
  return {
    props: {
      productData: productData ? JSON.parse(JSON.stringify(productData)) : null,
      purchasesData: purchasesData ? JSON.parse(JSON.stringify(purchasesData)) : [],
    },
  }
}
