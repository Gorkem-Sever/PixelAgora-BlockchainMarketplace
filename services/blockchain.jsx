...
import dappMrktAbi from '@/artifacts/contracts/DappMrkt.sol/DappMrkt.json'
...
const getEthereumContracts = async () => {
  ...
  if (accounts?.length > 0) {
    ...
    const contracts = new ethers.Contract(address.dappMrktContract, dappMrktAbi.abi, signer)
...
const buyProduct = async ({ aid, quantity, amount, timestamps }) => {
  if (!ethereum) {
    reportError('Please install a browser provider')
    return Promise.reject(new Error('Browser provider not installed'))
  }
  ...
  try {
    const contract = await getEthereumContracts()
    const tx = await contract.buyProduct(aid, quantity, timestamps, {
      value: toWei(amount),
    })
